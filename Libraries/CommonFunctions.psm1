...
Function DoTestCleanUp($CurrentTestResult, $testName, $DeployedServices, $ResourceGroups, [switch]$keepUserDirectory, [switch]$SkipVerifyKernelLogs)
{
	try
	{
		...
		if($ResourceGroups)
		{
			if(!$IsWindows){
				try
				{
					...
					#endregion
					$SQLQuery = Get-SQLQuery -TestPlatform $TestPlatform -TestLocation $TestLocation -TestCategory $TestCategory `
		...
		{
			$SQLQuery = Get-SQLQuery -TestPlatform $TestPlatform -TestLocation $TestLocation -TestCategory $TestCategory -TestArea $TestArea -TestName $CurrentTestData.TestName -CurrentTestResult $CurrentTestResult -ExecutionTag $ResultDBTestTag -GuestDistro $GuestDistro -KernelVersion $KernelVersion -LISVersion $LISVersion -HostVersion $HostVersion -VMSize $VMSize -Networking $Networking -ARMImage $ARMImage -OsVHD $OsVHD -BuildURL $env:BUILD_URL
			if($SQLQuery)
			{
				UploadTestResultToDatabase -SQLQuery $SQLQuery
			}
